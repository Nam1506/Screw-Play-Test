...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public void ForceResize(Vector2 value)
        {
            Debug.Log("resize");

            foreach (var panel in Panels)
            {
                panel.sizeDelta = value;
                Debug.Log("Panel: " + panel.sizeDelta, panel.gameObject);
            }
        }
        ...
        private void Setup()
        {
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                if (useAutomaticLayout)
                {
                    ...
                    Content.sizeDelta = new Vector2(contentWidth, /*movementAxis == MovementAxis.Vertical ? -contentHeight : */contentHeight);
                }
        ...
        {
            Debug.Log("GoToPanel: " + panelNumber);
        ...
        public void Custom_GoToNextPanel()
        {
            int nearestPanel = GetNearestPanel();
            if (nearestPanel != (NumberOfPanels - 1))
            {
        ...
        public void Remove(int index)
        {
            ...
            DestroyImmediate(Toggles[index].gameObject);
        ...
        private Vector2 GetDisplacementFromCenter(int index)
        {
            ...
            {
                //return Panels[index].anchoredPosition + Content.anchoredPosition - new Vector2(Viewport.rect.width * (0.1f - Content.anchorMin.x), Viewport.rect.height * (0.1f - Content.anchorMin.y));

                if (releaseDirection == Direction.Up)
                {
                    return Panels[index].anchoredPosition + Content.anchoredPosition - new Vector2(Viewport.rect.width * (0.05f - Content.anchorMin.x), Viewport.rect.height * (0.1f - Content.anchorMin.y));
                }
                else
                {
                    return Panels[index].anchoredPosition + Content.anchoredPosition - new Vector2(Viewport.rect.width * (0.95f - Content.anchorMin.x), Viewport.rect.height * (0.9f - Content.anchorMin.y));
                }
